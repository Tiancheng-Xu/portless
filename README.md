...
If a wrapped command self-daemonizes and exits while its assigned port is still listening, portless keeps the route and warns. Run `portless prune` to terminate the orphaned server and remove the retained route.
